...
const Onboarding = () => {
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      if (!formData.weight || Number(formData.weight) < 20 || Number(formData.weight) > 300) {
        return toast("Please enter a valid weight between 20 and 300.")
      } else if (formData.height) {
        if (Number(formData.height) < 100 || Number(formData.height) > 250) {
          return toast("Please enter a valid height between 100 and 250.")
        }
      }
    }
    if (step < totalSteps) {
      ...
      const existingUser = JSON.parse(localStorage.getItem('fitnessUser') || '{}');
      const userData = {
        ...existingUser,
        ...user,
        ...formData,
      ...
      console.log("Submitting Onboarding:", { userId: user?.id, userData });
      try {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isButtonActive = activeElement.tagName === 'BUTTON';

      if (e.key === 'Enter') {
        // Proceed if not focused on a button (to avoid double submission)
        if (!isButtonActive) {
          handleNext();
        }
      } else if (e.key === 'Backspace') {
        // Go back if not typing in an input
        if (!isInputActive && step > 1) {
          setStep(s => s - 1);
        }
      }
    };
        ...
        <div className="p-6 pt-12 onboarding-wrapper">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-slate-800 dark:text-white">ZenSpace</h1>
            </div>
            ...
            <div className="space-y-6 onboarding-wrapper">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Helps use track your progress</p>
                </div>
              </div>
              ...
              <div className="space-y-8 max-w-lg">
                ...
                <div className="space-y-6">
                  <Slider label="Daily Calorie Intake" min={120} max={4000} step={50}
                    value={formData.dailyCalorieIntake} onChange={(v) => updateField('dailyCalorieIntake', v)}
                  ...
                  <Slider label="Daily Calorie Burn" min={100} max={2000} step={50}
                    value={formData.dailyCalorieBurn} onChange={(v) => updateField('dailyCalorieBurn', v)}
